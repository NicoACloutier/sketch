...
def parameterize(vectors: np.ndarray, u_vals: np.ndarray) -> np.ndarray:
    """
    ...
    for vector in vectors:
        ...
        for vector_j in vectors:
            for i, point in enumerate(vector[:-1]):
                temp_intersect, temp_ind = intersections(vector_j, point, tangent(vector, i))
